...
@section('page-title')
    ...
    <p class="page-label" style="margin: 0; color: #777; font-size: 0.9rem;">Jelajahi kategori favoritmu dan temukan
        pengetahuan baru</p>
@endsection
...
@section('content')
    ...
    <div class="category-row"
        style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 16px; margin-bottom: 40px;">
        @php
        ...
        @forelse($categories as $index => $category)
            ...
            <div class="category-card {{ $cardColor }}" style="cursor: pointer;" data-target-slug="{{ $slug }}"
    ...
    <div class="books category-books" id="booksCategoryContainer">
        @forelse($books as $book)
            @php
                ...
                // Pengecekan apakah buku ini sudah ada di wishlist user saat ini
            ...
            <div class="book-card" data-category="{{ $bookCategorySlug }}">
                <div class="book-image" style="position: relative;">
                    ...
                    @else
                        ...
                        <div
                            style="width: 100%; height: 250px; border-radius: 8px; background: linear-gradient(135deg, #{{ $hexColor }}, #2c3e50); display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 15px; text-align: center; color: white; font-weight: bold;">
                ...
                <div class="book-meta"
                    ...
                    <!-- MODIFIKASI DISINI: Menambahkan data-id dan class dinamis berdasarkan status wishlist -->
                    <a class="bookmark-btn" data-id="{{ $book->id }}"
                        style="cursor: pointer; color: {{ $isWishlisted ? '#ff4757' : '#888' }}; font-size: 1.2rem;">
    ...
    <script>
        function selectCategory(slug, name) {
            ...
            if (visibleCount === 0) {
                if (!noMsg) {
                    ...
                    noMsg.style.cssText =
                        'grid-column: 1/-1; text-align: center; color: #888; padding: 40px 0; font-style: italic;';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Event Listener untuk Bookmark Toggle
            ...
            bookmarkButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    // Kirim request ke Laravel menggunakan Fetch API
                    fetch(`/member/wishlist/toggle/${bookId}`, {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            if (data.status === 'added') {
                                // Jika berhasil ditambah, ubah ikon jadi solid dan warna merah
                                icon.classList.remove('fa-regular');
                                icon.classList.add('fa-solid');
                                currentButton.style.color = '#ff4757';

                                // Integrasi Toastr jika kamu sudah memasangnya
                                if (typeof toastr !== 'undefined') toastr.success(data.message);
                            } else if (data.status === 'removed') {
                                // Jika dihapus, kembalikan ikon ke regular dan warna abu-abu
