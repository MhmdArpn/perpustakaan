...
<style>
    ...
    .book-cover-placeholder {
        width: 100%;
        height: 250px; /* Menyesuaikan dengan tinggi standar card img Anda */
        border-radius: 8px;
        display: flex;
    ...
    @forelse($popularBooks as $book)
        <div class="book-card">
            ...
            @else
                ...
                @endphp
                <div class="book-cover-placeholder" style="background: linear-gradient(135deg, #{{ $hexColor }}, #2c3e50);">
            ...
            <button {{ ($book->qty ?? 1) <= 0 ? 'disabled style=background:#ccc;cursor:not-allowed;' : '' }}>
                {{ ($book->qty ?? 1) > 0 ? 'Pinjam' : 'Habis' }}
            </button>
        </div>
    @empty
